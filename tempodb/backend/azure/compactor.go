...
func (rw *readerWriter) getAttributes(ctx context.Context, name string) (BlobAttributes, error) {
	...
	if props.ContentLength == nil {
		return BlobAttributes{}, fmt.Errorf("expected content length but got none for blob %s", name)
	}

	if props.LastModified == nil {
		return BlobAttributes{}, fmt.Errorf("expected last modified but got none for blob %s", name)
	}
